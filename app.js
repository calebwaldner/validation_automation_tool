...
const libraryDatabase = [
  ...
  },
  {
    name: "listDBFields",
    file: "list-fields.js",
    description: "Returns the fields from the database file, can be enhanced with arguments",
  ...
  },
  {
    name: "listSpecFields",
    file: "list-fields.js",
    description: "Returns the fields from the spec file, can be enhanced with arguments",
  ...
  },
  {
    name: "checkRequired",
    checkpoint: 3,
    dependencies: ["checkSpecColumnHeaders", "checkSpecLogicType", "matchSpecFieldNamesAndLabelNames"],
...
class Library {
  ...
  checkMessages() {
    checkMessages.run(this.databaseData, this.specData, this.arguments);
  }
}
  ...
  .then((specJSON) => { // Part of csvtojson module
    ...
    * Edit: I believe this could be altered to use the "done" event, which would fire once all data is in. Then I could use a global variable to capture the data\ and on the 'done' event do everything else */
    ...
    try {
      ...
      // TODO need to set up to allow this app to run again.
...
function normalizeDBData(dbJSON) {
  ...
  dbJSON.forEach(row => {
    const casArr = row["Conditional Actions"].split(regexLineSplit); //returns array of conditional actions
    casArr.forEach(ca => {
